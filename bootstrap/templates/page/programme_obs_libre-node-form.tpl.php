...
<div id='breadcrumPersoN65'>
	<a href="<?php echo $base_url; ?>" alt=''>Accueil</a> > <a href="<?php echo $base_url; ?>/liste-actions" alt=''>Actions</a> > <a href="<?php echo $base_url; ?>/a-vos-observations" alt=''>A vos observations !</a> > Saisie des données
</div>


<div class='container contentOfEditFormN65'>

	<div class='row-fluid' id='fold1InEditFormN65'>
		<div class='span12'>			
			<?php print drupal_render($form['field_date_observation_libre']); ?>
			<?php print drupal_render($form['field_heure_debut_libre']); ?>
			<?php print drupal_render($form['field_heure_fin_libre']); ?>
		</div>
	</div>
	...
	<div class='row-fluid'>
		...
		<div class='span4' id='leftColOnFold2OfEditFormN65'>
			<div class='row-fluid'>			
			<div class='span12'>
			<legend id='fieldPointRepere'><span class="fieldset-legend">Site d'observation</span></legend>
			<?php print drupal_render($form['field_zone_observation_libre']); ?>
			</div>
			</div>

			<div class='row-fluid'>	
			<div class='span12'>
			<?php print drupal_render($form['field_point_de_repere_libre']); ?>
			</div>
			</div>
		...
		<div class='span8'>
			<?php print drupal_render($form['field_localisations_libre']); ?>
		</div>
	...
	<div class='span8 offset4' id='fieldCoord'>
		<?php print drupal_render($form['field_coordonnees_libre']); ?>
...
<div class='row-fluid' id='mesPhotos'>
	...
	<div class='row-fluid' id='foldOfPhotoSiteEditFormN65'>
		<div class='span12'>
		<p class='labelOfPicturesEditForm'>Une photo générale du paysage littoral *</p>
		<?php print drupal_render($form['field_photo_lieu_libre']); ?>
		</div>
	</div>

	<div class='row-fluid' id='foldOfPhotoEspeceEditFormN65'>
		<div class='span12'>
		<p class='labelOfPicturesEditForm'>Photos des espèces rencontrées lors de votre sortie *</p>
		<?php print drupal_render($form['field_photo_especes_libre']); ?>
		</div>
	</div>
	</div>
</div>

	<div class='row-fluid' id='foldOfNoteEditFormN65'>
		<div class='span8'>
		<?php print drupal_render($form['field_note_libre']); ?>
		</div>
...
jQuery("#clickme2add").click(function() {
	jQuery("#fieldCoord").slideDown( "slow", function() {
		jQuery('div.contentOfEditFormN65 div#foldCoord').animate({paddingBottom:'16px'});
...
jQuery("#clickme2reduce").click(function() {
	...
	jQuery("#fieldCoord").slideUp( "slow", function() {
		jQuery('div.contentOfEditFormN65 div#foldCoord').animate({paddingBottom:'0px'});
...
jQuery('h1.page-header').text("À vos observations !");
jQuery('#field-date-observation-libre-add-more-wrapper label').text("Date d'observation*");
jQuery('.contentOfEditFormN65 button#edit-submit').text("Transmettre ma fiche d’observation");
jQuery('body.page-node-edit.node-type-programme-obs-libre #edit-submit').text("Enregistrer les modifications");
jQuery('#edit-title').val('À vos observations !');


jQuery('.contentOfEditFormN65 button#edit-submit').click(function(event) {
	...
	jQuery('#yesBtn').click(function() {
		jQuery('form.node-form.node-programme_obs_libre-form').submit();
	});
...
jQuery('#edit-field-localisations-libre input.geofield_wkt').each(function() {
    ...
    element.change(function(){
    	...
    	jQuery('#edit-field-coordonnees-libre input.geofield-lat').val(lon);
    	jQuery('#edit-field-coordonnees-libre input.geofield-lon').val(lat);
	})
...
jQuery('.btnMajMap').click(function() {
	...
	var lat = jQuery('#edit-field-coordonnees-libre input.geofield-lat').val();
	var lon = jQuery('#edit-field-coordonnees-libre input.geofield-lon').val();
